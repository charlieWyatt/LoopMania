...
public class LoopManiaApplication extends Application {
    ...
    public void start(Stage primaryStage) throws IOException {
        ...
        MainMenuController gameWonController = new MainMenuController();
        FXMLLoader wonLoader = new FXMLLoader(getClass().getResource("GameWonView.fxml"));
        wonLoader.setController(mainMenuController);
        Parent gameWonRoot = wonLoader.load();

        MainMenuController gameLostController = new MainMenuController();
        FXMLLoader lostLoader = new FXMLLoader(getClass().getResource("GameLostView.fxml"));
        lostLoader.setController(mainMenuController);
        Parent gameLostRoot = lostLoader.load();
        ...
        gameWonController.setGameSwitcher(() -> {
            switchToRoot(scene, gameRoot, primaryStage);
            mainController.startTimer();
        });

        mainController.setGameLostSwitcher(() -> {switchToRoot(scene, gameLostRoot, primaryStage);});
        gameLostController.setGameSwitcher(() -> {
            switchToRoot(scene, gameRoot, primaryStage);
            mainController.startTimer();
        });
